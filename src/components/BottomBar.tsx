...
export const BottomBar = ({ selectedTab }: { selectedTab: Tab | null }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-20 border-t-2 border-[#e5e5e5] bg-white shadow-lg md:hidden">
      <ul className="flex h-[70px] sm:h-[80px] items-center justify-around">
            ...
            >
              <Link
                ...
                className={`flex flex-col items-center justify-center transition-all duration-300 
                          ${isSelected 
                          ? "rounded-xl border-2 border-[#84d8ff] bg-[#ddf4ff] px-3 py-2 transform scale-105" 
                          : "px-3 py-2 hover:bg-gray-50 rounded-xl"}`}
              >
                <div className="flex items-center justify-center relative">
                  {item.icon}
                  {isSelected && (
                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-[#ff4b4b] rounded-full"></div>
                  )}
                </div>
                <span className={`text-xs font-medium mt-1 ${isSelected ? 'text-[#1cb0f6]' : 'text-gray-500'}`}>
                  {item.name}
                </span>
              </Link>
            </li>
