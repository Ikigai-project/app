...
export const LoginScreen = ({
  ...
  const logInAndSetUserProperties = () => {
    ...
    setName(name);
    // Définir automatiquement l'île IKIGAI pour l'utilisateur
    setLanguage({ 
      code: "equilibre", 
      name: "Découvrir son IKIGAI" 
    });
    logIn();
    
    // Redirection vers la page d'apprentissage après connexion
    setTimeout(() => {
  ...
  return (
    <article
      className={[
        "fixed inset-0 z-30 flex flex-col bg-white p-4 sm:p-6 md:p-7 transition duration-300",
    ...
    >
      ...
      <div className="flex grow items-center justify-center">
        <div className="flex w-full max-w-sm flex-col gap-4 sm:gap-5 px-1 sm:px-0 mx-auto">
          <h2 className="text-center text-xl sm:text-2xl font-bold text-gray-800">
            {loginScreenState === "LOGIN" ? "Se connecter" : "Créer votre profil"}
          </h2>
          ...
          >
            {loginScreenState === "LOGIN" ? "Se connecter" : "Créer un compte"}
          </button>
          ...
          <p className="text-center text-xs leading-5 text-gray-400">
            Ce site est protégé par reCAPTCHA Enterprise et la politique de confidentialité de Google{" "}
            <Link
              className="font-bold"
