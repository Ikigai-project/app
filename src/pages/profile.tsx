  ...
  return (
    <section className="flex flex-row-reverse border-b-2 border-gray-200 pb-8 md:flex-row md:gap-8">
      ...
      <div className="flex grow flex-col justify-between gap-3">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-3">
            <ProfileTimeJoinedSvg />
            <span className="text-gray-500">{`Inscrit depuis ${joinedAt}`}</span>
          </div>
          <div className="flex items-center gap-3">
            <ProfileFriendsSvg />
            <span className="text-gray-500">{`${followingCount} Suivis / ${followersCount} Abonnés`}</span>
          </div>
        </div>
      ...
      <Link
        ...
        className="hidden items-center gap-2 self-start rounded-2xl border-b-4 border-[#41D185] bg-[#41D185] px-5 py-3 font-bold uppercase text-white transition hover:brightness-110 md:flex"
      >
        <EditPencilSvg />
        Modifier le profil
      </Link>
...
const ProfileFriendsSection = () => {
  ...
  return (
    <section>
      <h2 className="mb-5 text-2xl font-bold">Amis</h2>
      <div className="rounded-2xl border-2 border-gray-200">
        <div className="flex">
          <button
            className={[
              "flex w-1/2 items-center justify-center border-b-2 py-3 font-bold uppercase hover:border-[#41D185] hover:text-[#41D185]",
              state === "FOLLOWING"
                ? "border-[#41D185] text-[#41D185]"
